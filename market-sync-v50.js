/* Asta Fantacalcio — Bacchetta definitiva v3.4.56
   ...
   - massimizza matematicamente il numero di slot riempiti, poi priorità/qualità
   - non supera mai il budget del ruolo
   - soluzione deterministica: dynamic programming
*/
(function(){
'use strict';
...
function candidates(r,priority,sold,budget){return all().filter(p=>p?.role===r&&hasObjective(p.id)&&priorityOf(p)===priority&&!sold.has(String(p.id))).map(p=>({p,cost:maxBid(p),market:marketEstimate(p),quality:quality(p)})).filter(c=>c.cost<=budget).sort((a,b)=>b.quality-a.quality||a.market-b.market||a.cost-b.cost||String(a.p.name||'').localeCompare(String(b.p.name||''),'it'))}
...
function solveRole(r,slotDefs,budget){
  ...
  const groups=ordered.map(d=>candidates(r,d.priority,sold,budget));
  let states=new Map();states.set(0,{cost:0,filled:0,value:0,chosen:[],used:new Set()});
  for(let i=0;i<groups.length;i++){
    const slot=ordered[i],next=new Map();
    for(const state of states.values()){
      const skip=next.get(state.cost);if(!skip||state.filled>skip.filled||state.filled===skip.filled&&state.value>skip.value)next.set(state.cost,state);
      for(const c of groups[i]){const cost=state.cost+c.cost;if(cost>budget)continue;const id=String(c.p.id);if(state.used.has(id))continue;const used=new Set(state.used);used.add(id);const candidate={cost,filled:state.filled+1,value:state.value+slotValue(c,slot.priority,budget),chosen:state.chosen.concat({...c,slot}),used};const prev=next.get(cost);if(!prev||candidate.filled>prev.filled||candidate.filled===prev.filled&&(candidate.value>prev.value||candidate.value===prev.value&&candidate.chosen.length>prev.chosen.length))next.set(cost,candidate)}
    }
    const compact=[...next.values()].sort((a,b)=>a.cost-b.cost||b.filled-a.filled||b.value-a.value);states=new Map();let bestFilled=-1,bestValue=-Infinity;for(const s of compact){if(s.filled>bestFilled||s.filled===bestFilled&&s.value>bestValue){states.set(s.cost,s);if(s.filled>bestFilled)bestFilled=s.filled;if(s.value>bestValue)bestValue=s.value}}
  }
  let best=null;for(const s of states.values())if(!best||s.filled>best.filled||s.filled===best.filled&&(s.value>best.value||s.value===best.value&&s.cost<best.cost))best=s;best=best||{chosen:[],cost:0,filled:0,value:0};return{ok:true,chosen:best.chosen,cost:best.cost,budget,filled:best.filled,total:ordered.length}
}
...
function install(){const version=document.querySelector('.app-version');if(version)version.textContent='V. '+VERSION;if(window.__ASTA_BACCHETTA_V13)return true;document.addEventListener('click',function(e){const btn=e.target?.closest?.('.magic-wand-btn');if(!btn)return;e.preventDefault();e.stopImmediatePropagation();const ctx=syncMarket(),originalPersist=typeof persist==='function'?persist:null;if(ctx&&originalPersist){persist=function(){const h=current.history;current.history=ctx.base;try{return originalPersist.apply(this,arguments)}finally{current.history=h}}}try{runNewWand(btn)}finally{if(originalPersist)persist=originalPersist;if(ctx)setTimeout(()=>{current.history=ctx.base},100)}},true);window.runMagicWand=runNewWand;if(typeof window.renderBrain==='function'&&!window.renderBrain.__marketSyncV13Wrapped){const original=window.renderBrain;window.renderBrain=function(){const s=activeStrategy();if(s)syncTargetAllocations(s);return original.apply(this,arguments)};window.renderBrain.__marketSyncV13Wrapped=true}window.__ASTA_BACCHETTA_V13=true;return true}
