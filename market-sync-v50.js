/* Asta Fantacalcio — Bacchetta definitiva v3.4.55
   ...
   - soluzione deterministica: dynamic programming, non beam search
*/
(function(){
'use strict';
...
function candidates(r,priority,sold){return all().filter(p=>p?.role===r&&hasObjective(p.id)&&priorityOf(p)===priority&&!sold.has(String(p.id))).map(p=>({p,cost:maxBid(p),market:marketEstimate(p),quality:quality(p)})).sort((a,b)=>b.quality-a.quality||a.market-b.market||a.cost-b.cost||String(a.p.name||'').localeCompare(String(b.p.name||''),'it'))}
...
/* Exact bounded-budget solver. State is keyed by spent credits. For every slot
   we may skip or assign one distinct candidate. The best state is chosen by:
   1) number of filled slots, 2) strategic value, 3) lower spend. */
function solveRole(r,slotDefs,budget){
  ...
  const groups=ordered.map(d=>candidates(r,d.priority,sold).slice(0,120));
  let states=new Map();
  states.set(0,{cost:0,filled:0,value:0,chosen:[],used:new Set()});
  for(let i=0;i<groups.length;i++){
    const slot=ordered[i],next=new Map();
    for(const state of states.values()){
      const skip=next.get(state.cost);
      if(!skip||state.filled>skip.filled||state.filled===skip.filled&&state.value>skip.value)next.set(state.cost,state);
      for(const c of groups[i]){
        const cost=state.cost+c.cost;if(cost>budget)continue;
        const id=String(c.p.id);if(state.used.has(id))continue;
        const used=new Set(state.used);used.add(id);
        const value=state.value+slotValue(c,slot.priority,budget);
        const candidate={cost,filled:state.filled+1,value,chosen:state.chosen.concat({...c,slot}),used};
        const prev=next.get(cost);
        if(!prev||candidate.filled>prev.filled||candidate.filled===prev.filled&&(candidate.value>prev.value||candidate.value===prev.value&&candidate.chosen.length>prev.chosen.length))next.set(cost,candidate);
      }
    }
    /* Dominance pruning: a state at higher cost is useless when another state
       costs less and already fills at least as many slots with equal/better value. */
    const compact=[...next.values()].sort((a,b)=>a.cost-b.cost||b.filled-a.filled||b.value-a.value);
    states=new Map();
    let bestFilled=-1,bestValue=-Infinity;
    for(const s of compact){
      if(s.filled>bestFilled||s.filled===bestFilled&&s.value>bestValue){states.set(s.cost,s);if(s.filled>bestFilled)bestFilled=s.filled;if(s.value>bestValue)bestValue=s.value;}
    }
  }
  let best=null;
  for(const s of states.values())if(!best||s.filled>best.filled||s.filled===best.filled&&(s.value>best.value||s.value===best.value&&s.cost<best.cost))best=s;
  best=best||{chosen:[],cost:0,filled:0,value:0};
  return{ok:true,chosen:best.chosen,cost:best.cost,budget,filled:best.filled,total:ordered.length}
}
