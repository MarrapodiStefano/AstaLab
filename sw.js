const CACHE = "asta-fantacalcio-v160";
...
self.addEventListener("fetch", event => {
 ...
 if(request.mode==="navigate"&&sameOrigin){
  event.respondWith(fetch(request,{cache:"no-store"}).then(async response=>{
   ...
   if(!injected.includes("ui-fixes.js"))injected=injected.replace(/<\/body>/i,'<script src="./ui-fixes.js?v=3.5.87"></script>\n</body>');
   if(!injected.includes("brain-engine.js"))injected=injected.replace(/<\/body>/i,'<script src="./brain-engine.js?v=3.5.91"></script>\n</body>');
