const CACHE = "asta-fantacalcio-v135";
    ...
    if(request.mode==="navigate" && sameOrigin){
        event.respondWith(
            fetch(request,{cache:"no-store"}).then(async response=>{
                ...
                let injected=html;
                if(!injected.includes("brain-sync-fix.js")) injected=injected.replace(/<\\/body>/i,'<script src="./brain-sync-fix.js?v=3.5.67" data-brain-sync-367></script>\n</body>');
                if(!injected.includes("brain-picker-fix.js")) injected=injected.replace(/<\\/body>/i,'<script src="./brain-picker-fix.js?v=3.5.67" data-brain-picker-367></script>\n</body>');
                if(!injected.includes("ui-fixes.js")) injected=injected.replace(/<\\/body>/i,'<script src="./ui-fixes.js?v=3.5.67" data-ui-fixes-367></script>\n</body>');
