/* Asta Fantacalcio — Bacchetta v9: budget guidato dal massimale del giocatore */
(function(){
'use strict';
...
/* credits = massimale che l'utente è disposto a spendere per quel giocatore. */
...
function slotValue(c,priority,budget){const rank=PRIORITY_RANK[priority]||1;const ratio=c.cost/Math.max(1,budget);/* La massima appetibilità ha precedenza: non viene sacrificata per risparmiare crediti. */if(rank>=5)return c.quality*100-ratio*.01;if(rank===4)return c.quality*15-ratio*.08;if(rank===3)return c.quality*5-ratio*.35;if(rank===2)return c.quality*2.5-ratio*.75;return c.quality-ratio*1.2;}
function solveRole(r,slotDefs,budget){
  /* Risolviamo prima gli slot più importanti. Questo fa sì che il massimale del giocatore della slot MAX venga riservato prima di distribuire il resto del budget. */
  ...
  let beam=[{chosen:[],used:new Set(),cost:0,value:0}],WIDTH=120;
  for(let i=0;i<groups.length;i++){
    ...
    for(const st of beam){
      ...
      for(const c of rows){
        ...
        const value=st.value+slotValue(c,ordered[i].priority,budget);
        next.push({chosen:st.chosen.concat({...c,slot:ordered[i]}),used,cost:total,value});
      }
    }
...
function buildPlan(s){
  const initial=Number(current?.initialCredits)||0,alloc=s?.allocation||{},team=typeof brainMyTeam==='function'?brainMyTeam():current?.teams?.[0],spentBy={P:0,D:0,C:0,A:0},countBy={P:0,D:0,C:0,A:0};
  (team?.players||[]).forEach(p=>{if(spentBy[p.role]!=null){spentBy[p.role]+=Number(p.price)||0;countBy[p.role]++;}});
  const plan={roles:{},errors:[]};
  ['P','D','C','A'].forEach(r=>{
    const total=slotCount(s,r),targets=targetsFor(s,r),defs=[];
...
function applyPlan(s,plan){
  ...
  const myPlayers=(typeof brainMyTeam==='function'?brainMyTeam():current?.teams?.[0])?.players||[];
  ['P','D','C','A'].forEach(r=>{
    const x=plan.roles[r],n=x.total,old=Array.isArray(s.slotTargets[r])?s.slotTargets[r]:[],planned=Math.max(1,initial*(Number(alloc[r])||0)/100),next=Array.from({length:n},(_,i)=>({priority:old[i]?.priority||'base',playerId:null})),pcts=Array.from({length:n},()=>0),bought=myPlayers.filter(p=>p.role===r);
    for(let i=0;i<x.count;i++){
      const p=bought[i];
      if(p){const bid=Math.max(1,Number(p.price)||0);next[i]={priority:old[i]?.priority||priorityOf(p),playerId:p.id};pcts[i]=Math.round(bid/planned*1000)/10;}
    }
    (x.solved?.chosen||[]).forEach(c=>{
      const bid=Math.max(1,Number(c.cost)||maxBid(c.p));
      next[c.slot.index]={priority:c.slot.priority,playerId:c.p.id};
      /* Il valore visualizzato dello slot è direttamente il massimale del giocatore / budget del ruolo. Kolo Muani: 360/600 = 60%. */
      pcts[c.slot.index]=Math.round(bid/planned*1000)/10;
    });
...
function runNewWand(btn){
  ...
  try{const plan=buildPlan(s);if(plan.errors.length){console.warn('Bacchetta v9',plan.errors);alert('La Bacchetta non può rispettare tutte le appetibilità con il budget attuale.\n\n'+plan.errors.join('\n'));return;}applyPlan(s,plan);console.info('Bacchetta v9: strategia ricalcolata con prenotazione dei massimali.',plan);}catch(e){console.error('Bacchetta v9',e);alert('Errore Bacchetta Magica: '+(e?.message||e));}finally{if(btn){btn.disabled=false;btn.classList.remove('magic-wand-running');btn.setAttribute('aria-busy','false');}}
}
...
function install(){
  ...
  if(window.__ASTA_BACCHETTA_V9)return true;
  document.addEventListener('click',function(e){
    const btn=e.target?.closest?.('.magic-wand-btn');if(!btn)return;
    e.preventDefault();e.stopImmediatePropagation();
    const ctx=syncMarket();const originalPersist=typeof persist==='function'?persist:null;
    if(ctx&&originalPersist){persist=function(){const h=current.history;current.history=ctx.base;try{return originalPersist.apply(this,arguments);}finally{current.history=h;}};}
    try{runNewWand(btn);}finally{if(originalPersist)persist=originalPersist;if(ctx)setTimeout(()=>{current.history=ctx.base;},100);}
  },true);
  window.runMagicWand=runNewWand;window.__ASTA_BACCHETTA_V9=true;return true;
}
