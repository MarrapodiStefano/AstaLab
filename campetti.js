...
function renderCampetti(){
  ...
  if(!grid) return;
  grid.innerHTML=CAMPI_SERIE_A.map(team=>`<button class="campetti-team campetti-${team.id}" type="button" onclick="openCampetto('${team.id}')" aria-label="Apri campetto ${team.name}"><span class="campetti-crest" aria-hidden="true"><img src="${team.crest}" alt="" loading="eager" referrerpolicy="no-referrer"></span></button>`).join('');
}
function openCampetto(teamId){
  const team=CAMPI_SERIE_A.find(t=>t.id===teamId); if(!team) return;
  const viewer=document.getElementById('campettiViewer'),image=document.getElementById('campettiViewerImage'),empty=document.getElementById('campettiViewerEmpty');
  if(!viewer||!image||!empty) return;
  viewer.classList.add('show'); document.body.classList.add('campetti-open'); campettiResetZoom();
  const campettiImages={atalanta:'./assets/atalanta.jpg',bologna:'./assets/bologna.PNG',cagliari:'./assets/cagliari.PNG',como:'./assets/como.PNG',fiorentina:'./assets/fiorentina.PNG',frosinone:'./assets/frosinone.PNG',genoa:'./assets/genoa.PNG',inter:'./assets/inter.PNG',juventus:'./assets/juventus.PNG',lazio:'./assets/lazio.PNG',lecce:'./assets/lecce.PNG',milan:'./assets/milan.jpg',monza:'./assets/monza.jpg',napoli:'./assets/napoli.jpg',parma:'./assets/parma.PNG',roma:'./assets/roma.PNG',sassuolo:'./assets/sassuolo.PNG',torino:'./assets/torino.PNG',udinese:'./assets/udinese.PNG',venezia:'./assets/venezia.PNG'};
  const imagePath=campettiImages[team.id];
  if(imagePath){image.src=imagePath+'?v=2.9.26';image.alt='Campetto '+team.name;image.style.display='block';empty.style.display='none';image.onerror=()=>{image.style.display='none';empty.innerHTML='<div class="campetti-empty-icon">⚽</div><b>Immagine '+team.name+' non trovata</b><span>Controlliamo insieme il nome del file nella cartella assets.</span>';empty.style.display='flex';};}
  else{image.removeAttribute('src');image.style.display='none';empty.innerHTML='<div class="campetti-empty-icon">⚽</div><b>Campetto '+team.name+'</b><span>Immagine non ancora collegata.</span>';empty.style.display='flex';}
}
...
/*
  FIX v3.4.27:
  carica Bacchetta Magica dopo app.js e mantiene la versione visibile aggiornata.
*/
(function loadMagicWand(){
  const versionEl=document.querySelector('.app-version');
  if(versionEl) versionEl.textContent='V. 3.4.27';
  if(document.querySelector('script[data-bacchetta]')) return;
  const script=document.createElement('script');
  script.src='./bacchetta.js?v=3.4.27';
