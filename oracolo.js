/* ORACOLO 1.0.10 — motore Smart 3.5.21 */
(function(){
'use strict';
...
function pool(s,r,used,priority){
  const sold=soldSet(s),obj=Array.isArray(s?.objectives)?s.objectives:[],objSet=new Set(obj.map(x=>String(x)));
  const all=players().filter(p=>p.role===r&&!sold.has(String(p.id))&&!used.has(String(p.id)));
  const exact=all.filter(p=>objSet.has(String(p.id))&&priorityOf(s,p.id)===priority);
  if(exact.length)return exact;
  const objective=all.filter(p=>objSet.has(String(p.id)));
  if(objective.length)return objective;
  return all;
}
function choose(s,st,r,i,used,slotCap,roleCap){
  const t=slotTargets(st,r)[i]||{priority:'base'},wanted=t.priority||'base';
  let list=pool(s,r,used,wanted),best=list.map(p=>({p,v:score(p,s,r,slotCap,wanted)})).filter(x=>Number.isFinite(x.v)).sort((a,b)=>b.v-a.v)[0]?.p||null;
  if(best)return best;
  list=pool(s,r,used,wanted);
  best=list.map(p=>({p,v:score(p,s,r,roleCap,wanted)})).filter(x=>Number.isFinite(x.v)).sort((a,b)=>b.v-a.v)[0]?.p||null;
  if(best)return best;
  const relaxed=['max','high','low','bet','base'].filter(x=>x!==wanted);
  for(const pr of relaxed){
    list=pool(s,r,used,pr);
    best=list.map(p=>({p,v:score(p,s,r,roleCap,pr)})).filter(x=>Number.isFinite(x.v)).sort((a,b)=>b.v-a.v)[0]?.p||null;
    if(best)return best;
  }
  return null;
}
...
function rememberBrainView(){try{const expanded=[...document.querySelectorAll('.brain-strategy.expanded')].map(c=>{const b=c.querySelector('.brain-strategy-name');const m=String(b?.getAttribute('onclick')||'').match(/toggleBrainStrategy\((\d+)\)/);return m?Number(m[1]):null}).filter(x=>x!=null);const slots=[...document.querySelectorAll('.brain-strategy.expanded .brain-role-row')].map(row=>{const b=row.querySelector('.brain-role-gear[aria-expanded="true"]');const m=String(b?.getAttribute('onclick')||'').match(/toggleBrainSlots\((\d+),\s*[\'\"]([PDCA])[\'\"]\)/);return m?{strategyId:Number(m[1]),role:m[2]}:null}).filter(Boolean);sessionStorage.setItem('AF_ORACOLO_VIEW',JSON.stringify({expanded,slots}))}catch(e){}}
function fill(){if(busy)return;const s=state(),st=strategy(s);if(!s||!st)return;busy=true;const mine=myTeam(s),used=new Set((mine?.players||[]).map(p=>String(p.id)));let changed=0;
  ROLES.forEach(r=>{const ts=slotTargets(st,r),pcts=slotPcts(st,r),planned=Math.round((Number(s.initialCredits)||1200)*(Number(st.allocation?.[r])||0)/100);let roleSpent=0;ts.forEach(t=>{if(!empty(t.playerId)){const p=players().find(x=>String(x.id)===String(t.playerId));if(p){used.add(String(p.id));roleSpent+=Number(p.credits)||0}}});
    ts.forEach((t,i)=>{if(!empty(t.playerId))return;const roleRemaining=Math.max(0,planned-roleSpent);if(roleRemaining<=0)return;const sb=Math.max(0,slotBudget(st,r,i,planned,pcts));const cap=Math.min(roleRemaining,sb>0?sb:roleRemaining);const p=choose(s,st,r,i,used,cap,roleRemaining);if(!p)return;const price=Number(p.credits)||0;if(price>roleRemaining)return;write(st,r,i,p);used.add(String(p.id));roleSpent+=price;changed++})
  });
  persist(s);setTimeout(()=>{busy=false;if(changed){rememberBrainView();sessionStorage.setItem('AF_ORACOLO_RETURN','brain');window.location.reload()}else alert('Oracolo non ha trovato giocatori compatibili con la strategia e il budget disponibile. Prova ad aumentare il budget di uno slot o a renderne meno stringente l’appetibilità.')},150)
}
