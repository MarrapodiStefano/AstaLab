...
(function(){
  ...
  function candidatesForSlot(s,r,i){
    ...
    const rows=players().filter(p=>p.role===r&&!sold.has(String(p.id))).map(p=>{const est=priceEstimate(p),score=playerScore(p,r),efficiency=score/Math.sqrt(Math.max(1,est)),preferred=selected!=null&&String(selected)===String(p.id)?1.15:1;return {p,est,score,efficiency,preferred};});
  ...
  function currentRoleStats(){const out={P:{count:0,spent:0},D:{count:0,spent:0},C:{count:0,spent:0},A:{count:0,spent:0}},t=myTeam();(t?.players||[]).forEach(p=>{if(out[p.role]){out[p.role].count++;out[p.role].spent+=Number(p.price)||0;}});return out;}
  function roleBaseBudgets(s,totalBudget){const a=s?.allocation||{},sum=['P','D','C','A'].reduce((n,r)=>n+Math.max(0,Number(a[r])||0),0)||100,out={};['P','D','C','A'].forEach(r=>out[r]=totalBudget*Math.max(0,Number(a[r])||0)/sum);return out;}
  function makeSlots(s){
    ...
    for(const r of ['P','D','C','A']){
      ...
      for(let i=0;i<totalSlots&&added<need;i++){if(i<stats[r].count)continue;const target=targetFor(s,r,i);slots.push({r,i,pct:Number(pcts[i])||0,budget:roleBudget[r]*(Number(pcts[i])||0)/100,priority:target.priority||'base'});added++;}
    }
  ...
  function optimize(s){
    ...
    for(const item of pool){
      const next=[];
      for(const state of beam){for(const c of item.cands){
      ...
      const seen=new Set(),ded=[];for(const st of next){const key=st.chosen.map(x=>x.p.id).sort().join(',');if(seen.has(key))continue;seen.add(key);ded.push(st);if(ded.length>=BEAM_WIDTH)break;}beam=ded;
    }
  ...
  function applyToActiveStrategy(result){
    ...
    const byRole={P:[],D:[],C:[],A:[]};result.best.chosen.forEach(x=>byRole[x.slot.r].push(x));
    ['P','D','C','A'].forEach(r=>{const targets=typeof brainSlotTargets==='function'?brainSlotTargets(s,r):[];byRole[r].forEach(x=>{const i=x.slot.i;if(targets[i])targets[i].playerId=x.p.id;});s.slotTargets[r]=targets;});
